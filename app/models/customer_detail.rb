  ...
  def update(params)
    Customer.transaction do
      ...
      Address.find(self.billing_address_id).update(
                           address_attributes(params.permit(:billing_street, :billing_city, :billing_state, :billing_zipcode), "billing"))

      Address.find(self.shipping_address_id).update(
                           address_attributes(params.permit(:shipping_street, :shipping_city, :shipping_state, :shipping_zipcode), "shipping"))
    end
  end
